...
class ControllerPaymentPayeer extends Controller {
	protected function index() 
	{
		...
		$this->data['m_orderid'] = $this->session->data['payeer_order_id'];
		$this->data['m_amount'] = $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false);
		$this->data['m_curr'] = $order_info['currency_code'];
		$this->data['m_desc'] = base64_encode('Payment # ' . $this->data['m_orderid']);
		$arHash = array(
			$this->data['m_shop'],
			$this->data['m_orderid'],
			$this->data['m_amount'],
	...
	public function status() 
	{
		if (isset($_POST["m_operation_id"]) && isset($_POST["m_sign"]))
		{
			...
			if ($this->config->get('payeer_log_value') == 1)
			{
				file_put_contents($_SERVER['DOCUMENT_ROOT'].'/system/logs/payeer_log.txt', $log_text, FILE_APPEND);
			}
			...
			if ($_POST["m_sign"] == $sign_hash && $_POST['m_status'] == "success" && $valid_ip)
			{
				...
				$this->load->model('checkout/order');
				$this->model_checkout_order->confirm($order_id, $this->config->get('payeer_order_status_id'));
